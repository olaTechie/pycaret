...
| 20 | pycaret.internal.pycaret_experiment.tabular_experiment / schemdraw | tests/smoke/test_plotting.py::test_classification_plot[pipeline] (and reg/clu siblings) | `TypeError: Drawing.draw() got an unexpected keyword argument 'ax'` | schemdraw | plotting-dev | open | Empirical, Phase 3 smoke baseline (commit `10901cd9`). schemdraw 0.16 (lifted in commit `50979d29`) removed the `ax=` kwarg from `Drawing.draw()`; pycaret's `pipeline` plot dispatcher at `tabular_experiment.py:569` still passes `ax=...`. Fix: rewrite the call site to use schemdraw 0.16's documented API (`Drawing.draw()` returns the figure; pass the desired axes via the Drawing constructor or by inserting onto an existing figure). |
| 21 | pycaret.internal.plots / yellowbrick | tests/smoke/test_plotting.py::test_regression_plot[error] (and `residuals`) | `yellowbrick.exceptions.YellowbrickTypeError: This estimator is not a regressor` | yellowbrick | plotting-dev | open | Empirical, Phase 3 smoke baseline (commit `10901cd9`). Regression-side sibling of row 18: yellowbrick's `is_regressor` probes pycaret's wrapped Pipeline rather than the inner estimator. Same fix path as row 18 — pipeline-unwrap shim in `pycaret/internal/patches/yellowbrick.py`. |
| 22 | pycaret.internal.plots / yellowbrick | tests/smoke/test_plotting.py::test_clustering_plot[distance] (and `elbow`, `silhouette`) | `yellowbrick.exceptions.YellowbrickTypeError: The supplied model is not a clustering estimator` | yellowbrick | plotting-dev | open | Empirical, Phase 3 smoke baseline (commit `10901cd9`). Clustering-side sibling of row 18: yellowbrick's clustering type check rejects pycaret's wrapped Pipeline. Same fix path as row 18 — pipeline-unwrap shim. |
